...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        sample = [XX[i] for i in sample_index]
        samples = [XXi[:-1] for XXi in sample] # sampled dataset
        labels = [XXi[-1] for XXi in sample]  # class labels for the sampled records
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                y = np.append(y,np.random.choice(2)) # randomize the label
            else:
